lineage_pyrate<-function(x){
  ...
  fixed_clade_key<-data.frame(clade_name<<-x.1$clade,clade_no<<-as.numeric(factor(x.1$clade))) %>% group_by(clade_name) %>% summarize(No<<-first(clade_no), count<<-n()) %>% arrange(No)
